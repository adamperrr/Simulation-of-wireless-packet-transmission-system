...
public class SendFromTransmitterEvent implements ConditionalEvent
{
  ...
  {
    // SendFromTransmitterEvent doesn't check if channel is free because it is
    // what listening do
  ...
  {
    if (condition())
    { // If there's any ready transmitter to send packet
      ...
      { // Loop looks for a ready transmitter to send packet
        ...
        {
          Packet tempPacket = tempTrans.transmissionSettings.getProcesedPacket(); // Gets
                                                                                  // packet
                                                                                  // currently
                                                                                  // processed
                                                                                  // in
          ...
          if (tempPacket != null)
          {
            ...
            channel.setBusy(); // Setting channel busy - listening events will
                               // see this
            ...
            {
              System.out
                  .println("-> SendFromTransmitterEvent: Packet sent to channel from transmitter " + tempTrans.getId());
              System.out.println("--> passTime: " + tempTrans.transmissionSettings.getPassTime());
            }
            transmissionSystem.addTimeEvent(new ArrivalToTheReceiverEvent(transmissionSystem.getClock(),
                tempPacket.getAddress(), transmissionSystem));
            transmissionSystem.addTimeEvent(
                new ACKarrivalEvent(transmissionSystem.getClock(), tempPacket.getAddress(), transmissionSystem));
  ...
  private Vector<Transmitter> transmitters = null; // Vectors of the
