...
/**
 * Class representing conditional event of starting processing new packet from
 * transmitter buffer
 */
public class ProcessingNewPacketEvent implements ConditionalEvent
{
  ...
  /**
   * Conditions which should return true to execute event It returns true when
   * there are packets to process in the buffer
   */
  ...
  /**
   * Execution of Event If conditions of event are met: Algorithm looks for a
   * transmitters which aren't busy and sets them busy and sets their
   * ProcesedPacket.
   */
  public void execute()
  {
        ...
        if (tempTrans != null && !tempTrans.isBusy())
        {
          ...
          if (tempPacket != null)
          {
            ...
            if (numOfPacketRetrans <= maxRetrans)
            {
              ...
              {
                System.out.println(
                    "-> ProcessingNewPacketEvent: Starting ProcessingNewPacket in transmitter " + tempTrans.getId());
              }

              transmissionSystem.addTimeEvent(
                  new ListenIfChannelIsFreeEvent(transmissionSystem.getClock(), tempTrans, transmissionSystem));
            }
          }
        }
      }
