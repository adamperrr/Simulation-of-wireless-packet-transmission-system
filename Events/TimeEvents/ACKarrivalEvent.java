...
public class ACKarrivalEvent implements TimeEvent
{
  ...
  /**
   * Event planed by SendFromTransmitterEvent It pushes packet ACK to the proper
   * transmitter or activates retransmission
   */
  public void execute()
  {
    ...
    if (ACK != null)
    {
      ...
      {
        System.out.println(
            time + ": ACKarrivalEvent: ACK packet arrived to the transmitter " + packetAddress + " from the channel.");
      }
    } else
    {
      if (TransmissionSystem.logsON)
      {
        System.out.println(
            time + ": ACKarrivalEvent: The ACK has not arrived. There will be a retransmission attempt for the transmitter "
                + packetAddress);
      }
      ...
      transmissionSystem
          .addTimeEvent(new ListenIfChannelIsFreeEvent(retransListeningStartTime, transmitter, transmissionSystem));
    }
