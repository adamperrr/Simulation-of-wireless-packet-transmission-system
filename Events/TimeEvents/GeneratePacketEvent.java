...
/**
 * Class GeneratePacketEvent Generates a packet in the specified transmitter on
 * random time since moment of initialization.
 */
public class GeneratePacketEvent implements TimeEvent
{
  ...
  /**
   * Describes all of the actions made by event Function pushes packet to the
   * buffer of transmitter specified in the variable 'transmitter'. It also
   * generates next event of type GeneratePacketEvent and adds it to the
   * agenda/timeEvents.
   */
  public void execute()
  {
    ...
    {
      System.out.println(
          time + ": GeneratePacketEvent: Packet pushed to the transmitter's " + transmitter.getId() + " buffer");
    }
    ...
    {
      System.out.println(
          time + ": GeneratePacketEvent: Planned next GeneratePacketEvent for transmitter " + transmitter.getId());
    }
  }
